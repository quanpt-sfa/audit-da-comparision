...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run CFS item mapping and observed shifting-proxy validation")
    ...
    for key in ["raw_input", "panel_input"]:
        path = resolve(repo_root, config["paths"][key])
        if not path.exists():
            raise FileNotFoundError(f"Required input not found: {path}")
    output = resolve(repo_root, config["paths"]["output_dir"])
    upstream_name = config.get("upstream", {}).get("observed_cases_table", "cfs_offset_channel_cases")
    if not (output / f"{upstream_name}.csv").exists() and not (output / f"{upstream_name}.csv.gz").exists():
