...
def applied_consequence_tables(
    ...
    if lag_assets:
        pair["__log_assets"] = np.log(
            np.maximum(pd.to_numeric(pair[lag_assets], errors="coerce").abs(), 1.0)
        )
    elif "assets_pre" in pair:
        pair["__log_assets"] = np.log(
            np.maximum(pd.to_numeric(pair.assets_pre, errors="coerce").abs(), 1.0)
        )
    else:
    ...
    aliases = {
        ...
        "short_debt": [
            "__short_debt_intensity",
            "short_term_debt_intensity_pre",
            "std_intensity_pre",
        ],
        "loss": ["loss_pre"],
        "roa": ["roa_pre"],
    ...
    for model, group in primary.groupby("model", observed=True):
        ...
        for focal_name in ["big4", "short_debt", "loss"]:
            ...
            controls = ["__log_assets"] + [
                ...
                if name not in {focal_name} and column is not None
            ]
            for outcome_name, pre_col, post_col in [
                ...
                continuous = [
                    ...
                    for column in {
                        "__log_assets",
                        resolved.get("short_debt"),
                        resolved.get("roa"),
                        resolved.get("current_ratio"),
                    }
                ...
                x, names = _design_matrix(
                    d, [focal, *controls], industry_col=industry_col
                )
                focal_idx = names.index(focal)
                b_pre, se_pre, p_pre = _cluster_ols(
                    d[pre_col].to_numpy(float), x, d.issuer_ticker.to_numpy()
                )
                b_post, se_post, p_post = _cluster_ols(
                    d[post_col].to_numpy(float), x, d.issuer_ticker.to_numpy()
                )

                stacked = pd.concat(
                    [
                        d.assign(__y=d[pre_col], __post=0.0, __interaction=0.0),
                        d.assign(
                            __y=d[post_col],
                            __post=1.0,
                            __interaction=pd.to_numeric(d[focal], errors="coerce"),
                        ),
                    ],
                    ignore_index=True,
                )
                stacked["__focal"] = pd.to_numeric(stacked[focal], errors="coerce")
                xs, snames = _design_matrix(
                    stacked,
                    ["__focal", "__post", "__interaction", *controls],
                    industry_col=industry_col,
                )
                inter_idx = snames.index("__interaction")
                b_st, se_st, p_st = _cluster_ols(
                    stacked.__y.to_numpy(float),
                    xs,
                    stacked.issuer_ticker.to_numpy(),
                )

                d["__diff"] = d[post_col] - d[pre_col]
                xd, dnames = _design_matrix(
                    d, [focal, *controls], industry_col=industry_col
                )
                diff_idx = dnames.index(focal)
                b_diff, se_diff, p_diff = _cluster_ols(
                    d.__diff.to_numpy(float), xd, d.issuer_ticker.to_numpy()
                )
                ...
                rows.append(
                    {
                        ...
                        "n": len(d),
                        "issuers": d.issuer_ticker.nunique(),
                        "pre_beta": b_pre[focal_idx],
                        "pre_se": se_pre[focal_idx],
                        "pre_p": p_pre[focal_idx],
                        "post_beta": b_post[focal_idx],
                        "post_se": se_post[focal_idx],
                        "post_p": p_post[focal_idx],
                        "beta_difference": b_post[focal_idx] - b_pre[focal_idx],
                        "standardised_beta_difference": (
                            (b_post[focal_idx] - b_pre[focal_idx]) / pre_sd
                            if pre_sd > 0
                            else np.nan
                        ),
                        "interaction_beta": b_st[inter_idx],
                        "interaction_se": se_st[inter_idx],
                        "interaction_p": p_st[inter_idx],
                        "paired_difference_beta": b_diff[diff_idx],
                        "paired_difference_se": se_diff[diff_idx],
                        "paired_difference_p": p_diff[diff_idx],
                        "significance_status_switch": (
                            p_pre[focal_idx] < 0.05
                        ) != (p_post[focal_idx] < 0.05),
                    }
                )
                manifest.append(
                    {
    ...
    if not out.empty:
        out["interaction_q_bh"] = _adjust_pvalues(
            out.interaction_p.fillna(1.0), "bh"
        )
        out["paired_difference_q_bh"] = _adjust_pvalues(
            out.paired_difference_p.fillna(1.0), "bh"
        )
    return out, pd.DataFrame(manifest)


def supplemental_inference(
    concentration: pd.DataFrame | None,
    near_zero: pd.DataFrame | None,
    settings: CompletionSettings,
) -> pd.DataFrame:
    rows: list[dict] = []
    if concentration is not None and (not concentration.empty) and "excess_nhhi" in concentration:
        ...
        boot = cluster_bootstrap(
            x,
            lambda z: float(z.excess_nhhi.mean()),
        ...
        rows.append(
            {
                "diagnostic": "excess_nhhi",
                **boot,
                "share_positive": float(x.excess_nhhi.gt(0).mean()),
                "n": len(x),
            }
        )
    if near_zero is not None and (not near_zero.empty) and {"draw", "statistic"}.issubset(near_zero):
        observed_rows = near_zero.loc[
            near_zero.draw.astype(str).eq("observed"), "statistic"
        ]
        simulated = pd.to_numeric(
            near_zero.loc[~near_zero.draw.astype(str).eq("observed"), "statistic"],
            errors="coerce",
        ...
        if len(observed_rows) and len(simulated):
            ...
            rows.append(
                {
                    "diagnostic": "near_zero_random_sign",
                    "estimate": observed,
                    "ci_low": float(np.quantile(simulated, 0.025)),
                    "ci_high": float(np.quantile(simulated, 0.975)),
                    "p_directional": _mc_p(abs(observed), np.abs(simulated)),
                    "n": len(simulated),
                }
            )
